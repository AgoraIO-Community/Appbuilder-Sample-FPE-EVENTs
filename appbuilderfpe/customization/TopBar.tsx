import {customEvents, useRender, useLocalUserInfo} from 'customization-api';
...
const Timer = () => {
  ...
  const [shuffledArray, setShuffledUids] = useState(activeUids);
  const [sender, setSender] = useState(null);

  useEffect(() => {
    setShuffledUids(shuffled(activeUids));
  }, [activeUids]);
  ...
  useEffect(() => {
    ...
    if (counter === 0 && sender && uid === sender) {
      customEvents.send(
        'generate-sequence',
        JSON.stringify({
          shuffledUids: shuffledArray,
        }),
  ...
  useEffect(() => {
    let interval = null;
    customEvents.on('start-timer', ({payload, sender}) => {
      let data = JSON.parse(payload);
      setCounter(data.startTime);
      // start the game timer and send the timer started acknowledge to sender
      customEvents.send(
        'timer-started',
        JSON.stringify({
          startTime: 20,
    ...
    // only received by the person who started the game
    let interval1 = null;
    customEvents.on('timer-started', ({payload}) => {
      let data = JSON.parse(payload);
      setCounter(data.startTime);
